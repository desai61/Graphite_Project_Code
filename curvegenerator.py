...
######## USER INPUTS ########
nrx = 30     #Number of repeat units in x dir (actual will have nrx - 0.5)
R = 5     #Radius of sheet (circular cap)
...
################# HORIZONTAL SHEETS #################
def horizontal_sheet(xstart, ystart, xlength, nry, z, na):
    x = xstart   
    y1 = np.arange(ystart+bl/2,nry*3*bl,3*bl)
    y2 = np.arange(ystart+3*bl/2,nry*3*bl,3*bl)
    y3 = insort(y1,y2)
    y4 = y3 + 3*bl/2
    ylen = len(y3)
    i = 0
    while(x < xlength):
        if (i%2 == 1):
            coords.append([x,ystart,z])
            na += 1
            write_coords(coords, na)
        for j in range(ylen):
            if(i%2 == 0):
                coords.append([x,y3[j],z])
                na += 1
                write_coords(coords, na)
            else:                
                coords.append([x,y4[j],z])
                na += 1
                write_coords(coords, na)
        if (i%2 == 0):
            coords.append([x,y4[ylen-1]+bl/2,z])
            na += 1
            write_coords(coords, na)
        i += 1
        x += la/2
...
############## FUNCTION CALLS ################
xhor, yhor, zhor, na = horizontal_sheet(0,bl,nrx*la,nry,0,na)
